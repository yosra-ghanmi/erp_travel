...
const defaultClient = { name: '', email: '', phone: '', country: '', notes: '' }

export function ClientsPage({ clients, setClients, bookings, canDelete, agencyId }) {
  ...
  const saveClient = async () => {
    ...
    try {
      if (editingId) {
        // Mock update for now
        setClients((prev) => prev.map((item) => (item.id === editingId ? { ...item, ...form } : item)))
      } else {
      ...
      if (!editingId) {
        ...
        setClients((prev) => [...prev, { id: fallbackId, agency_id: agencyId, ...form }])
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-3">
      <Panel title="Clients List" right={<p className="text-xs text-slate-500">{filtered.length} records</p>}>
            ...
            <tr key={client.id} className="border-b border-slate-100 dark:border-slate-800">
              ...
              <td className="px-2 py-3">
                <div className="flex gap-2">
                  <Button variant="ghost" onClick={() => selectClient(client)}>Edit</Button>
                  {canDelete ? <Button variant="danger" onClick={() => deleteClient(client.id)}>Delete</Button> : null}
                </div>
              </td>
            </tr>
          ))}
      ...
      <Panel title={editingId ? 'Edit Client' : 'Add Client'}>
        <div className="space-y-3">
          ...
          <textarea className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-brand-500 focus:ring-2 dark:border-slate-700 dark:bg-slate-950" rows={4} value={form.notes} onChange={(e) => setForm((prev) => ({ ...prev, notes: e.target.value }))} placeholder="Notes" />
          <Button onClick={saveClient} className="w-full" disabled={loading}>
            {loading ? 'Processing...' : (editingId ? 'Update Client' : 'Create Client')}
          </Button>
        </div>
      </Panel>
      ...
      <Panel title="Client Profile">
        <div className="space-y-3">
          <p className="text-sm text-slate-500">Select a client from the table with Edit to open profile details.</p>
          {editingId ? (
            <>
              ...
              <div>
                <p className="mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">Booking History</p>
                <div className="space-y-2">
                  {bookings.filter((booking) => booking.clientId === editingId).map((booking) => (
                    <div key={booking.id} className="rounded-lg border border-slate-200 p-2 text-xs dark:border-slate-700">
                      {booking.destination} ({booking.startDate}) - {booking.status}
                    </div>
                  ))}
                </div>
              </div>
