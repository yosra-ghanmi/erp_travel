...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    const shouldSyncWithSystem = (() => {
      try {
        return !window.localStorage.getItem("navigo-login-theme");
      } catch {
        return true;
      }
    })();
        ...
        <div className="flex min-h-screen w-full items-stretch lg:h-full">
          <div className="w-full max-w-[1280px] overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all duration-300 dark:border-slate-800 dark:bg-slate-900 lg:mx-auto lg:flex lg:h-[640px] lg:max-h-[88vh] lg:items-stretch">
            <section className="relative hidden h-full flex-1 overflow-hidden lg:block">
              <div
                className="absolute inset-0 bg-cover bg-center transition-transform duration-[12s] will-change-transform"
              ...
              <div className="relative z-10 flex h-full flex-col justify-between p-10 text-white xl:p-12">
                <div>
                  ...
                  <h1 className="mt-8 max-w-lg text-3xl font-extrabold leading-tight tracking-[-0.01em]">
                    Elevate your Travel Business Smarter{" "}
                    <span className="text-[#F97316]">with AI.</span>
                  </h1>
                  <p className="mt-3 max-w-md text-sm leading-6 text-slate-100/90">
                    Experience the future of agency management. Intelligent,
                    unified, and stunningly simple.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="relative rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl xl:p-5">
                    ...
                    <p className="mt-2 text-sm text-slate-100">
                      Generate day-by-day trips in seconds.
                    </p>
                  </div>
                  <div className="relative rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl xl:p-5">
                    <span className="absolute right-4 top-4 h-2.5 w-2.5 rounded-full bg-[#F97316] shadow-[0_0_10px_rgba(249,115,22,0.85)]" />
                    ...
                    <p className="mt-2 text-sm text-slate-100">
                      Seamless CRM & ERP features to manage clients, bookings,
            ...
            <section className="relative h-full min-h-0 w-full overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden bg-white p-4 transition-all duration-300 ease-in-out dark:bg-slate-900 sm:p-6 lg:flex-1 lg:p-8">
              <div className="absolute right-4 top-4 z-20 flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 sm:right-6 sm:top-6">
                ...
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white/80 text-slate-600 shadow-soft transition hover:bg-white dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-200 dark:hover:bg-slate-800"
                >
                  {darkMode ? (
              ...
              <div className="relative mx-auto flex h-full w-full max-w-md flex-col justify-center py-3">
                <MotionDiv {...motionItem} className="mb-3 mt-1 text-center">
                  ...
                  <h2 className="mt-1.5 text-xl font-bold tracking-[-0.01em] text-slate-800 dark:text-white">
                    Sign in to Navigo ERP
                  </h2>
                  <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400 sm:text-sm">
                    Use your agency account to continue in your AI-powered
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      />
                      {isEmailValid ? (
                        <CircleCheck className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-500" />
                      ) : null}
                      {isEmailInvalid ? (
                        <CircleX className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-rose-500" />
                      ) : null}
                    </div>
                    {errors.email ? (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      />
                      {isPasswordValid ? (
                        <CircleCheck className="pointer-events-none absolute right-10 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-500" />
                      ) : null}
                      {isPasswordInvalid ? (
                        <CircleX className="pointer-events-none absolute right-10 top-1/2 h-4 w-4 -translate-y-1/2 text-rose-500" />
                      ) : null}
                      <button
                        type="button"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link
                      to="/forgot-password"
                      className="text-sm font-semibold text-[#F97316] transition-all duration-300 ease-in-out hover:text-orange-600"
                    >
                      Forgot password?
                    </Link>
                  </div>
