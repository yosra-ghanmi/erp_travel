...
export function FinancialDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [pData, eData] = await Promise.all([fetchPayments(), fetchExpenses()])
        setPayments(pData || [])
        setExpenses(eData || [])
      } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    return { totalRevenue, totalExpenses, profit, monthlyData, expenseDistribution }
  }, [payments, expenses])
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card title="Total Revenue" value={`$${stats.totalRevenue.toLocaleString()}`} icon={DollarSign} hint="Total payments received" />
        <Card title="Total Expenses" value={`$${stats.totalExpenses.toLocaleString()}`} icon={TrendingDown} hint="Operating costs" />
        <Card title="Net Profit" value={`$${stats.profit.toLocaleString()}`} icon={TrendingUp} hint="Revenue minus expenses" />
        <Card title="Profit Margin" value={`${stats.totalRevenue > 0 ? Math.round((stats.profit / stats.totalRevenue) * 100) : 0}%`} icon={Landmark} hint="Efficiency ratio" />
      </div>
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-4">
            {stats.expenseDistribution.map((item, index) => (
              <div key={item.name} className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full" style={{ backgroundColor: COLORS[index % 5] }} />
                <span className="text-xs text-slate-600 dark:text-slate-400">{item.name} (${item.value.toLocaleString()})</span>
              </div>
            ))}
          </div>
        </Panel>
