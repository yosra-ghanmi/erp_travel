...
export function BookingsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [bData, cData] = await Promise.all([fetchBookings(), fetchClients()])
        setBookings(bData || [])
        setClients(cData || [])
      } catch (err) {
  ...
  const saveBooking = async () => {
    ...
    try {
      if (editingId) {
        // Mock update
        setBookings((prev) => prev.map((item) => (item.bookingid === editingId ? { ...item, ...form } : item)))
      } else {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-3">
      <div className="xl:col-span-2">
        <Panel title="Bookings Table" right={<p className="text-xs text-slate-500">{bookings.length} records</p>}>
          {error && <div className="mb-4 text-xs text-rose-500 font-medium">{error}</div>}
          <DataTable
            headers={['ID', 'Client', 'Trip', 'Dates', 'Amount', 'Actions']}
            rows={bookings.map((booking) => (
              <tr key={booking.bookingid} className="border-b border-slate-100 dark:border-slate-800">
                <td className="px-2 py-3 text-xs font-mono">{booking.bookingid}</td>
                ...
                <td className="px-2 py-3 text-xs">{booking.startdate} → {booking.enddate}</td>
                <td className="px-2 py-3 font-bold">${booking.amount}</td>
                <td className="px-2 py-3">
                  <div className="flex gap-2">
                    <Button variant="ghost" onClick={() => startEdit(booking)}>Edit</Button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Panel title={editingId ? 'Edit Booking' : 'Create Booking'}>
        <div className="space-y-3">
          ...
          <Input placeholder="Trip Name" value={form.tripName} onChange={(e) => setForm((prev) => ({ ...prev, tripName: e.target.value }))} />
          ...
          <Input type="number" value={form.amount} onChange={(e) => setForm((prev) => ({ ...prev, amount: e.target.value }))} placeholder="Amount" />
          <textarea 
          ...
          <Button onClick={saveBooking} className="w-full" disabled={loading}>
            {loading ? 'Processing...' : (editingId ? 'Update Booking' : 'Save Booking')}
          </Button>
        </div>
      </Panel>
    </div>
