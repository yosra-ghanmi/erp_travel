...
export function TripsPage({ trips, setTrips, bookings, searchQuery }) {
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter((trip) => {
      ...
      const services = String(trip.services || "").toLowerCase();
      return (
        title.includes(q) || destination.includes(q) || services.includes(q)
      );
    });
  }, [trips, searchQuery]);
          ...
          <DataTable
            ...
            rows={filteredTrips.map((trip) => (
              ...
              >
                ...
                <td className="px-2 py-3">
                  {
                    bookings.filter((booking) => booking.tripId === trip.id)
                      .length
                  }
                </td>
              </tr>
      ...
      <Panel title="Create Package">
        <div className="space-y-3">
          ...
          <Input
            ...
            onChange={(e) =>
              setForm((prev) => ({ ...prev, seatsLeft: e.target.value }))
            }
          />
