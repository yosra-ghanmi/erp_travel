  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between">
        ...
        {Icon ? <Icon className="h-5 w-5 text-brand-600 dark:text-brand-500" /> : null}
      </div>
...
export function StatusBadge({ value }) {
  const palette = {
    ...
    active: 'bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-400',
  ...
  return (
    <span className={`rounded-full px-2 py-1 text-xs font-medium ${palette[value] ?? palette.lead}`}>
      {value}
    </span>
  )
}
...
export function Button({ variant = 'primary', className = '', ...props }) {
  ...
  return (
    <button
      className={`rounded-xl px-3 py-2 text-sm font-medium transition ${variants[variant] ?? variants.primary} ${className}`}
      {...props}
    />
  )
}
      ...
      <table className="min-w-full text-left text-sm">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-800">
            {headers.map((header) => (
              <th key={header} className="px-2 py-3 font-medium text-slate-500 dark:text-slate-400">{header}</th>
            ))}
          </tr>
        </thead>
        <tbody>{rows}</tbody>
